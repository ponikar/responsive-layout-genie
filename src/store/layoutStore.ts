...
import { uploadZipBlob } from "@/lib/firebase";
...
export interface LayoutState {
  ...
  importConfig: (config: {
    ...
    assets: { [key: string]: AssetMetadata };
  }) => void;
...
export const useLayoutStore = create<LayoutState>((set, get) => ({
  ...
  addContainer: (parentId?: string) => {
    const parent = parentId
      ? get().containers.find((c) => c.id === parentId)
      : null;
  ...
  deleteContainer: (id) => {
    const newContainers = get().containers.filter(
  ...
  getExportData: () => {
    ...
    const processContainer = (
      ...
      const result: ExportedContainer = {
        portrait: {
          x: parentDimensions
            ? (container.portrait.x - parentDimensions.portrait.x) /
              parentDimensions.portrait.width
            : container.portrait.x / device.width,
          y: parentDimensions
            ? (container.portrait.y - parentDimensions.portrait.y) /
              parentDimensions.portrait.height
        ...
        landscape: {
          x: parentDimensions
            ? (container.landscape.x - parentDimensions.landscape.x) /
              parentDimensions.landscape.width
            : container.landscape.x / device.height,
          y: parentDimensions
            ? (container.landscape.y - parentDimensions.landscape.y) /
              parentDimensions.landscape.height
      ...
      if (Object.keys(container.assets).length > 0) {
        result.assets = Object.entries(container.assets).reduce(
          (acc, [id, asset]) => ({
            ...acc,
            [id]: {
              ...asset,
    ...
    const flattenContainers = (
      ...
      return Object.entries(containers).flatMap(([name, container]) => {
        ...
        const flatContainer: Container = {
          ...
          portrait: {
            x: parentDimensions
              ? parentDimensions.portrait.x +
                container.portrait.x * parentDimensions.portrait.width
              : container.portrait.x * selectedDevice.width,
            y: parentDimensions
              ? parentDimensions.portrait.y +
                container.portrait.y * parentDimensions.portrait.height
          ...
          landscape: {
            x: parentDimensions
              ? parentDimensions.landscape.x +
                container.landscape.x * parentDimensions.landscape.width
              : container.landscape.x * selectedDevice.height,
            y: parentDimensions
              ? parentDimensions.landscape.y +
                container.landscape.y * parentDimensions.landscape.height
          ...
          assets: container.assets
            ? Object.entries(container.assets).reduce(
                (acc, [id, asset]) => ({
                  ...acc,
                  [id]: {
                    ...asset,
  ...
  exportLayout: async () => {
    ...
    try {
      ...
      const imagePromises = Object.entries(uploadedImages).map(
        async ([assetId, dataUrl]) => {
          if (!dataUrl) return;
      ...
      console.log("DOWNLOADING ZIPPPP", url);
    } catch (error) {
  ...
  setActiveOrientation: (orientation) =>
    set({ activeOrientation: orientation }),
  ...
  reorderContainer: (
    ...
    if (position === "inside") {
      ...
      container.depth = state.getNextAvailableDepth(
        containers[targetIndex].parentId
      );
  ...
  updateContainerParent: (containerId: string, newParentId: string | null) => {
    ...
    const oldParent = oldParentId
      ? containers.find((c) => c.id === oldParentId)
      : null;
    const newParent = newParentId
      ? containers.find((c) => c.id === newParentId)
      : null;
    ...
    const absolutePortrait = oldParent
      ? {
          x:
            container.portrait.x +
            (oldParent.portrait.x - oldParent.portrait.width / 2),
          y:
            container.portrait.y +
            (oldParent.portrait.y - oldParent.portrait.height / 2),
    ...
    const absoluteLandscape = oldParent
      ? {
          x:
            container.landscape.x +
            (oldParent.landscape.x - oldParent.landscape.width / 2),
          y:
            container.landscape.y +
            (oldParent.landscape.y - oldParent.landscape.height / 2),
    ...
    const newPortrait = newParent
      ? {
          x:
            absolutePortrait.x -
            (newParent.portrait.x - newParent.portrait.width / 2),
          y:
            absolutePortrait.y -
            (newParent.portrait.y - newParent.portrait.height / 2),
    ...
    const newLandscape = newParent
      ? {
          x:
            absoluteLandscape.x -
            (newParent.landscape.x - newParent.landscape.width / 2),
          y:
            absoluteLandscape.y -
            (newParent.landscape.y - newParent.landscape.height / 2),
    ...
    const newContainers = get().containers.map((c) =>
      c.id === id
        ? {
            ...c,
            isLocked: !c.isLocked,
            assets: Object.entries(c.assets).reduce(
              (acc, [assetId, asset]) => ({
                ...acc,
                [assetId]: {
                  ...asset,
  ...
  getAbsoluteDepth: (containerId: string, assetId?: string) => {
    ...
    while (current) {
      ...
      current = current.parentId
        ? containers.find((c) => c.id === current.parentId)
        : null;
    }
